...
for release in releases['pipelines']:
    ...
    automated_test_package_id = boomi_cicd.query_packaged_component(automated_test_component_id, package_version)
    ...
    if not package_id and not automated_test_package_id:
        ...
        request_id = boomi_cicd.create_execution_request(atom_id, automated_test_component_id)
        ...
        if execution_response['status'] != "COMPLETE":
            raise AssertionError(f"Automation test for {process_name}. Error message: {execution_response['message']}")
